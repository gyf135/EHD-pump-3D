...
__global__ void gpu_initialization(double *r, double *c, double *cn, double *fi, double *u, double *v, double *w, double *ex, double *ey, double *ez, double *temp)
{
	...
	if (z==0){
		...
		fi[sidx] = 0.0;// voltage*(Lz - dz*z) / Lz;
	}
	u[sidx] = 0.0;
	v[sidx] = 0.0;
	w[sidx] = 0.0;
	...
	//if (dz*z > Lz / 2 + 0.0*Lz*cos(dx*x * 2 * M_PI / LL)) { // 
	//	temp[sidx] = 0.0;
	//}
	//else {
	//	temp[sidx] = 1.0;
...
__host__ void init_equilibrium(double *f0, double *f1, double *h0, double *h1, double *hn0, double *hn1, double *temp0, double *temp1, double *phi0, double *phi1,
	double *r, double *c, double *cn, double *u, double *v, double *w, double *ex, double *ey, double *ez, double *temp, double *phi)
{
    // blocks in grid
    dim3  grid(NX/nThreads, NY, NZ);
    // threads in block
    ...
    gpu_init_equilibrium<<< grid, threads >>>(f0,f1,h0,h1, hn0,hn1,temp0,temp1, phi0, phi1, r,c,cn,u,v,w,ex,ey,ez,temp,phi);
	...
	for (unsigned int i = 0; i <= N_in_phi; ++i) {

		printf("Iteration for initial phi: %d\n", i);
	
		//gpu_PBE << <grid, threads >> > (c, phi, cn);
	
		// =========================================================================
		// Fast poisson solver
		// =========================================================================
		//fast_Poisson(c, cn, kx, ky, kz, plan);

		// =========================================================================
		// LBM poisson solver
		// =========================================================================
		//for (int it = 0; it < N_in_phi; it++) {
			LBM_Poisson(phi0_gpu, phi1_gpu, phi2_gpu, c, cn, phi, t);
		//}
...
__global__ void gpu_init_equilibrium(double *f0, double *f1, double *h0, double *h1, double *hn0, double *hn1, double *temp0, double *temp1, double *phi0, double *phi1,
	double *r, double *c, double *cn, double *u, double *v, double *w, double *ex, double *ey, double *ez, double*temp, double*phi)
{
    unsigned int y = blockIdx.y;
	unsigned int z = blockIdx.z;
	...
	double p	  = phi[gpu_scalar_index(x, y, z)];
	...
	// zero weight
	phi0[gpu_field0_index(x, y, z)] = w0p;
	

	// adjacent weight
	// phi
	phi1[gpu_fieldn_index(x, y, z, 1)] = wsp;
	phi1[gpu_fieldn_index(x, y, z, 2)] = wsp;
	phi1[gpu_fieldn_index(x, y, z, 3)] = wsp;
	phi1[gpu_fieldn_index(x, y, z, 4)] = wsp;
	phi1[gpu_fieldn_index(x, y, z, 5)] = wsp;
	phi1[gpu_fieldn_index(x, y, z, 6)] = wsp;

	// diagonal weight
	// phi
	phi1[gpu_fieldn_index(x, y, z, 7)] = wap;
	phi1[gpu_fieldn_index(x, y, z, 8)] = wap;
	phi1[gpu_fieldn_index(x, y, z, 9)] = wap;
	phi1[gpu_fieldn_index(x, y, z, 10)] = wap;
	phi1[gpu_fieldn_index(x, y, z, 11)] = wap;
	phi1[gpu_fieldn_index(x, y, z, 12)] = wap;
	phi1[gpu_fieldn_index(x, y, z, 13)] = wap;
	phi1[gpu_fieldn_index(x, y, z, 14)] = wap;
	phi1[gpu_fieldn_index(x, y, z, 15)] = wap;
	phi1[gpu_fieldn_index(x, y, z, 16)] = wap;
	phi1[gpu_fieldn_index(x, y, z, 17)] = wap;
	phi1[gpu_fieldn_index(x, y, z, 18)] = wap;

	// 3d diagonal
	phi1[gpu_fieldn_index(x, y, z, 19)] = wdp;
	phi1[gpu_fieldn_index(x, y, z, 20)] = wdp;
	phi1[gpu_fieldn_index(x, y, z, 21)] = wdp;
	phi1[gpu_fieldn_index(x, y, z, 22)] = wdp;
	phi1[gpu_fieldn_index(x, y, z, 23)] = wdp;
	phi1[gpu_fieldn_index(x, y, z, 24)] = wdp;
	phi1[gpu_fieldn_index(x, y, z, 25)] = wdp;
	phi1[gpu_fieldn_index(x, y, z, 26)] = wdp;
}
	...
	double *r, double *c, double *cn, double *u, double *v, double *w, double *ex, double *ey, double *ez, double *Temperature, double t,double *f0bc)
{
	...
	double charge = ht0 + ht1 + ht2 + ht3 + ht4 + ht5 + ht6 + ht7 + ht8 + ht9
		+ ht10 + ht11 + ht12 + ht13 + ht14 + ht15 + ht16 + ht17 + ht18 + ht19 + ht20 + ht21 + ht22 + ht23 + ht24 + ht25 + ht26 + sourcep*dt*0.5;
	double chargen = hnt0 + hnt1 + hnt2 + hnt3 + hnt4 + hnt5 + hnt6 + hnt7 + hnt8 + hnt9
		+ hnt10 + hnt11 + hnt12 + hnt13 + hnt14 + hnt15 + hnt16 + hnt17 + hnt18 + hnt19 + hnt20 + hnt21 + hnt22 + hnt23 + hnt24 + hnt25 + hnt26 + sourcen*dt*0.5;
	...
	if (perturb==1){
		...
		// Apply scalar
		//if (zz > Lz / 2 + 0.0*Lz*cos(dx*x * 2 * M_PI / LL)) { // 
		//	temp = 0.0;
		//}
		//else {
		//	temp = 1.0;
	...
	// calculate ion source population
	// temperory variables
	double coe0p = w0 * sourcep;
	double coesp = ws * sourcep;
	double coeap = wa * sourcep;
	double coedp = wd * sourcep;
	...
	double fpopp8 = coeap;
	double fpopp9 = coeap;
	double fpopp10 = coeap;
	double fpopp11 = coeap;
	double fpopp12 = coeap;

	double fpopp13 = coeap;
	double fpopp14 = coeap;
	double fpopp15 = coeap;
	double fpopp16 = coeap;
	double fpopp17 = coeap;
	double fpopp18 = coeap;

	double fpopp19 = coedp;
	double fpopp20 = coedp;
	double fpopp21 = coedp;
	double fpopp22 = coedp;
	double fpopp23 = coedp;
	double fpopp24 = coedp;
	double fpopp25 = coedp;
	double fpopp26 = coedp;

	// temperory variables
	double coe0n = w0 * sourcen;
	double coesn = ws * sourcen;
	double coean = wa * sourcen;
	double coedn = wd * sourcen;
	...
	double fpopn8 = coean;
	double fpopn9 = coean;
	double fpopn10 = coean;
	double fpopn11 = coean;
	double fpopn12 = coean;

	double fpopn13 = coean;
	double fpopn14 = coean;
	double fpopn15 = coean;
	double fpopn16 = coean;
	double fpopn17 = coean;
	double fpopn18 = coean;

	double fpopn19 = coedn;
	double fpopn20 = coedn;
	double fpopn21 = coedn;
	double fpopn22 = coedn;
	double fpopn23 = coedn;
	double fpopn24 = coedn;
	double fpopn25 = coedn;
	double fpopn26 = coedn;
	...
	// source of ions
	// calculate source_plus and source_minus
	double forcepp0 = fpopp0;
	double forcepp1 = 0.5*(fpopp1 + fpopp2);
	double forcepp2 = forcepp1;
	double forcepp3 = 0.5*(fpopp3 + fpopp4);
	double forcepp4 = forcepp3;
	double forcepp5 = 0.5*(fpopp5 + fpopp6);
	double forcepp6 = forcepp5;
	double forcepp7 = 0.5*(fpopp7 + fpopp8);
	double forcepp8 = forcepp7;
	double forcepp9 = 0.5*(fpopp9 + fpopp10);
	double forcepp10 = forcepp9;
	double forcepp11 = 0.5*(fpopp11 + fpopp12);
	double forcepp12 = forcepp11;
	double forcepp13 = 0.5*(fpopp13 + fpopp14);
	double forcepp14 = forcepp13;
	double forcepp15 = 0.5*(fpopp15 + fpopp16);
	double forcepp16 = forcepp15;
	double forcepp17 = 0.5*(fpopp17 + fpopp18);
	double forcepp18 = forcepp17;
	double forcepp19 = 0.5*(fpopp19 + fpopp20);
	double forcepp20 = forcepp19;
	double forcepp21 = 0.5*(fpopp21 + fpopp22);
	double forcepp22 = forcepp21;
	double forcepp23 = 0.5*(fpopp23 + fpopp24);
	double forcepp24 = forcepp23;
	double forcepp25 = 0.5*(fpopp25 + fpopp26);
	double forcepp26 = forcepp25;

	double forcemp0 = 0.0;
	double forcemp1 = 0.5*(fpopp1 - fpopp2);
	double forcemp2 = -forcemp1;
	double forcemp3 = 0.5*(fpopp3 - fpopp4);
	double forcemp4 = -forcemp3;
	double forcemp5 = 0.5*(fpopp5 - fpopp6);
	double forcemp6 = -forcemp5;
	double forcemp7 = 0.5*(fpopp7 - fpopp8);
	double forcemp8 = -forcemp7;
	double forcemp9 = 0.5*(fpopp9 - fpopp10);
	double forcemp10 = -forcemp9;
	double forcemp11 = 0.5*(fpopp11 - fpopp12);
	double forcemp12 = -forcemp11;
	double forcemp13 = 0.5*(fpopp13 - fpopp14);
	double forcemp14 = -forcemp13;
	double forcemp15 = 0.5*(fpopp15 - fpopp16);
	double forcemp16 = -forcemp15;
	double forcemp17 = 0.5*(fpopp17 - fpopp18);
	double forcemp18 = -forcemp17;
	double forcemp19 = 0.5*(fpopp19 - fpopp20);
	double forcemp20 = -forcemp19;
	double forcemp21 = 0.5*(fpopp21 - fpopp22);
	double forcemp22 = -forcemp21;
	double forcemp23 = 0.5*(fpopp23 - fpopp24);
	double forcemp24 = -forcemp23;
	double forcemp25 = 0.5*(fpopp25 - fpopp26);
	double forcemp26 = -forcemp25;

	double spp = 1.0 - 0.5*dt*omega_c_plus;
	double smp = 1.0 - 0.5*dt*omega_c_minus;

	double sourcep0 = spp*fpopp0;
	double sourcep1 = spp*forcepp1 + smp*forcemp1;
	double sourcep2 = spp*forcepp2 + smp*forcemp2;
	double sourcep3 = spp*forcepp3 + smp*forcemp3;
	double sourcep4 = spp*forcepp4 + smp*forcemp4;
	double sourcep5 = spp*forcepp5 + smp*forcemp5;
	double sourcep6 = spp*forcepp6 + smp*forcemp6;
	double sourcep7 = spp*forcepp7 + smp*forcemp7;
	double sourcep8 = spp*forcepp8 + smp*forcemp8;
	double sourcep9 = spp*forcepp9 + smp*forcemp9;
	double sourcep10 = spp*forcepp10 + smp*forcemp10;
	double sourcep11 = spp*forcepp11 + smp*forcemp11;
	double sourcep12 = spp*forcepp12 + smp*forcemp12;
	double sourcep13 = spp*forcepp13 + smp*forcemp13;
	double sourcep14 = spp*forcepp14 + smp*forcemp14;
	double sourcep15 = spp*forcepp15 + smp*forcemp15;
	double sourcep16 = spp*forcepp16 + smp*forcemp16;
	double sourcep17 = spp*forcepp17 + smp*forcemp17;
	double sourcep18 = spp*forcepp18 + smp*forcemp18;
	double sourcep19 = spp*forcepp19 + smp*forcemp19;
	double sourcep20 = spp*forcepp20 + smp*forcemp20;
	double sourcep21 = spp*forcepp21 + smp*forcemp21;
	double sourcep22 = spp*forcepp22 + smp*forcemp22;
	double sourcep23 = spp*forcepp23 + smp*forcemp23;
	double sourcep24 = spp*forcepp24 + smp*forcemp24;
	double sourcep25 = spp*forcepp25 + smp*forcemp25;
	double sourcep26 = spp*forcepp26 + smp*forcemp26;

	// calculate source_neg_plus and source_neg_minus
	double forcepn0 = fpopn0;
	double forcepn1 = 0.5*(fpopn1 + fpopn2);
	double forcepn2 = forcepn1;
	double forcepn3 = 0.5*(fpopn3 + fpopn4);
	double forcepn4 = forcepn3;
	double forcepn5 = 0.5*(fpopn5 + fpopn6);
	double forcepn6 = forcepn5;
	double forcepn7 = 0.5*(fpopn7 + fpopn8);
	double forcepn8 = forcepn7;
	double forcepn9 = 0.5*(fpopn9 + fpopn10);
	double forcepn10 = forcepn9;
	double forcepn11 = 0.5*(fpopn11 + fpopn12);
	double forcepn12 = forcepn11;
	double forcepn13 = 0.5*(fpopn13 + fpopn14);
	double forcepn14 = forcepn13;
	double forcepn15 = 0.5*(fpopn15 + fpopn16);
	double forcepn16 = forcepn15;
	double forcepn17 = 0.5*(fpopn17 + fpopn18);
	double forcepn18 = forcepn17;
	double forcepn19 = 0.5*(fpopn19 + fpopn20);
	double forcepn20 = forcepn19;
	double forcepn21 = 0.5*(fpopn21 + fpopn22);
	double forcepn22 = forcepn21;
	double forcepn23 = 0.5*(fpopn23 + fpopn24);
	double forcepn24 = forcepn23;
	double forcepn25 = 0.5*(fpopn25 + fpopn26);
	double forcepn26 = forcepn25;

	double forcemn0 = 0.0;
	double forcemn1 = 0.5*(fpopn1 - fpopn2);
	double forcemn2 = -forcemn1;
	double forcemn3 = 0.5*(fpopn3 - fpopn4);
	double forcemn4 = -forcemn3;
	double forcemn5 = 0.5*(fpopn5 - fpopn6);
	double forcemn6 = -forcemn5;
	double forcemn7 = 0.5*(fpopn7 - fpopn8);
	double forcemn8 = -forcemn7;
	double forcemn9 = 0.5*(fpopn9 - fpopn10);
	double forcemn10 = -forcemn9;
	double forcemn11 = 0.5*(fpopn11 - fpopn12);
	double forcemn12 = -forcemn11;
	double forcemn13 = 0.5*(fpopn13 - fpopn14);
	double forcemn14 = -forcemn13;
	double forcemn15 = 0.5*(fpopn15 - fpopn16);
	double forcemn16 = -forcemn15;
	double forcemn17 = 0.5*(fpopn17 - fpopn18);
	double forcemn18 = -forcemn17;
	double forcemn19 = 0.5*(fpopn19 - fpopn20);
	double forcemn20 = -forcemn19;
	double forcemn21 = 0.5*(fpopn21 - fpopn22);
	double forcemn22 = -forcemn21;
	double forcemn23 = 0.5*(fpopn23 - fpopn24);
	double forcemn24 = -forcemn23;
	double forcemn25 = 0.5*(fpopn25 - fpopn26);
	double forcemn26 = -forcemn25;

	double spn = 1.0 - 0.5*dt*omega_c_plus;
	double smn = 1.0 - 0.5*dt*omega_c_minus;

	double sourcen0 = spn*fpopn0;
	double sourcen1 = spn*forcepn1 + smn*forcemn1;
	double sourcen2 = spn*forcepn2 + smn*forcemn2;
	double sourcen3 = spn*forcepn3 + smn*forcemn3;
	double sourcen4 = spn*forcepn4 + smn*forcemn4;
	double sourcen5 = spn*forcepn5 + smn*forcemn5;
	double sourcen6 = spn*forcepn6 + smn*forcemn6;
	double sourcen7 = spn*forcepn7 + smn*forcemn7;
	double sourcen8 = spn*forcepn8 + smn*forcemn8;
	double sourcen9 = spn*forcepn9 + smn*forcemn9;
	double sourcen10 = spn*forcepn10 + smn*forcemn10;
	double sourcen11 = spn*forcepn11 + smn*forcemn11;
	double sourcen12 = spn*forcepn12 + smn*forcemn12;
	double sourcen13 = spn*forcepn13 + smn*forcemn13;
	double sourcen14 = spn*forcepn14 + smn*forcemn14;
	double sourcen15 = spn*forcepn15 + smn*forcemn15;
	double sourcen16 = spn*forcepn16 + smn*forcemn16;
	double sourcen17 = spn*forcepn17 + smn*forcemn17;
	double sourcen18 = spn*forcepn18 + smn*forcemn18;
	double sourcen19 = spn*forcepn19 + smn*forcemn19;
	double sourcen20 = spn*forcepn20 + smn*forcemn20;
	double sourcen21 = spn*forcepn21 + smn*forcemn21;
	double sourcen22 = spn*forcepn22 + smn*forcemn22;
	double sourcen23 = spn*forcepn23 + smn*forcemn23;
	double sourcen24 = spn*forcepn24 + smn*forcemn24;
	double sourcen25 = spn*forcepn25 + smn*forcemn25;
	double sourcen26 = spn*forcepn26 + smn*forcemn26;
	...
	h0[gpu_field0_index(x, y, z)] = ht0 - (tw0cp * (hp0 - hep0) + tw0cm * (hm0 - hem0)) + dt*sourcep0;
	hn0[gpu_field0_index(x, y, z)] = hnt0 - (tw0cnp * (hnp0 - hnep0) + tw0cnm * (hnm0 - hnem0)) + dt*sourcen0;
	...
	h2[gpu_fieldn_index(x, y, z, 1)] = ht1 - (tw0cp * (hp1 - hep1) + tw0cm * (hm1 - hem1)) + dt*sourcep1;
	hn2[gpu_fieldn_index(x, y, z, 1)] = hnt1 - (tw0cnp * (hnp1 - hnep1) + tw0cnm * (hnm1 - hnem1)) + dt*sourcen1;
	...
	h2[gpu_fieldn_index(x, y, z, 2)] = ht2 - (tw0cp * (hp2 - hep2) + tw0cm * (hm2 - hem2)) + dt*sourcep2;
	hn2[gpu_fieldn_index(x, y, z, 2)] = hnt2 - (tw0cnp * (hnp2 - hnep2) + tw0cnm * (hnm2 - hnem2)) + dt*sourcen2;
	...
	h2[gpu_fieldn_index(x, y, z, 3)] = ht3 - (tw0cp * (hp3 - hep3) + tw0cm * (hm3 - hem3)) + +dt*sourcep3;
	hn2[gpu_fieldn_index(x, y, z, 3)] = hnt3 - (tw0cnp * (hnp3 - hnep3) + tw0cnm * (hnm3 - hnem3)) + dt*sourcen3;
	...
	h2[gpu_fieldn_index(x, y, z, 4)] = ht4 - (tw0cp * (hp4 - hep4) + tw0cm * (hm4 - hem4)) + dt*sourcep4;
	hn2[gpu_fieldn_index(x, y, z, 4)] = hnt4 - (tw0cnp * (hnp4 - hnep4) + tw0cnm * (hnm4 - hnem4)) + dt*sourcen4;
	...
	h2[gpu_fieldn_index(x, y, z, 5)] = ht5 - (tw0cp * (hp5 - hep5) + tw0cm * (hm5 - hem5)) + dt*sourcep5;
	hn2[gpu_fieldn_index(x, y, z, 5)] = hnt5 - (tw0cnp * (hnp5 - hnep5) + tw0cnm * (hnm5 - hnem5)) + dt*sourcen5;
	...
	h2[gpu_fieldn_index(x, y, z, 6)] = ht6 - (tw0cp * (hp6 - hep6) + tw0cm * (hm6 - hem6)) + dt*sourcep6;
	hn2[gpu_fieldn_index(x, y, z, 6)] = hnt6 - (tw0cnp * (hnp6 - hnep6) + tw0cnm * (hnm6 - hnem6)) + dt*sourcen6;
	...
	h2[gpu_fieldn_index(x, y, z, 7)] = ht7 - (tw0cp * (hp7 - hep7) + tw0cm * (hm7 - hem7)) + dt*sourcep7;
	hn2[gpu_fieldn_index(x, y, z, 7)] = hnt7 - (tw0cnp * (hnp7 - hnep7) + tw0cnm * (hnm7 - hnem7)) + dt*sourcen7;
	...
	h2[gpu_fieldn_index(x, y, z, 8)] = ht8 - (tw0cp * (hp8 - hep8) + tw0cm * (hm8 - hem8)) + dt*sourcep8;
	hn2[gpu_fieldn_index(x, y, z, 8)] = hnt8 - (tw0cnp * (hnp8 - hnep8) + tw0cnm * (hnm8 - hnem8)) + dt*sourcen8;
	...
	h2[gpu_fieldn_index(x, y, z, 9)] = ht9 - (tw0cp * (hp9 - hep9) + tw0cm * (hm9 - hem9)) + dt*sourcep9;
	hn2[gpu_fieldn_index(x, y, z, 9)] = hnt9 - (tw0cnp * (hnp9 - hnep9) + tw0cnm * (hnm9 - hnem9)) + dt*sourcen9;
	...
	h2[gpu_fieldn_index(x, y, z, 10)] = ht10 - (tw0cp * (hp10 - hep10) + tw0cm * (hm10 - hem10)) + dt*sourcep10;
	hn2[gpu_fieldn_index(x, y, z, 10)] = hnt10 - (tw0cnp * (hnp10 - hnep10) + tw0cnm * (hnm10 - hnem10)) + dt*sourcen10;
	...
	h2[gpu_fieldn_index(x, y, z, 11)] = ht11 - (tw0cp * (hp11 - hep11) + tw0cm * (hm11 - hem11)) + dt*sourcep11;
	hn2[gpu_fieldn_index(x, y, z, 11)] = hnt11 - (tw0cnp * (hnp11 - hnep11) + tw0cnm * (hnm11 - hnem11)) + dt*sourcen11;
	...
	h2[gpu_fieldn_index(x, y, z, 12)] = ht12 - (tw0cp * (hp12 - hep12) + tw0cm * (hm12 - hem12)) + dt*sourcep12;
	hn2[gpu_fieldn_index(x, y, z, 12)] = hnt12 - (tw0cnp * (hnp12 - hnep12) + tw0cnm * (hnm12 - hnem12)) + dt*sourcen12;
	...
	h2[gpu_fieldn_index(x, y, z, 13)] = ht13 - (tw0cp * (hp13 - hep13) + tw0cm * (hm13 - hem13)) + dt*sourcep13;
	hn2[gpu_fieldn_index(x, y, z, 13)] = hnt13 - (tw0cnp * (hnp13 - hnep13) + tw0cnm * (hnm13 - hnem13)) + dt*sourcen13;
	...
	h2[gpu_fieldn_index(x, y, z, 14)] = ht14 - (tw0cp * (hp14 - hep14) + tw0cm * (hm14 - hem14)) + dt*sourcep14;
	hn2[gpu_fieldn_index(x, y, z, 14)] = hnt14 - (tw0cnp * (hnp14 - hnep14) + tw0cnm * (hnm14 - hnem14)) + dt*sourcen14;
	...
	h2[gpu_fieldn_index(x, y, z, 15)] = ht15 - (tw0cp * (hp15 - hep15) + tw0cm * (hm15 - hem15)) + dt*sourcep15;
	hn2[gpu_fieldn_index(x, y, z, 15)] = hnt15 - (tw0cnp * (hnp15 - hnep15) + tw0cnm * (hnm15 - hnem15)) + dt*sourcen15;
	...
	h2[gpu_fieldn_index(x, y, z, 16)] = ht16 - (tw0cp * (hp16 - hep16) + tw0cm * (hm16 - hem16)) + dt*sourcep16;
	hn2[gpu_fieldn_index(x, y, z, 16)] = hnt16 - (tw0cnp * (hnp16 - hnep16) + tw0cnm * (hnm16 - hnem16)) + dt*sourcen16;
	...
	h2[gpu_fieldn_index(x, y, z, 17)] = ht17 - (tw0cp * (hp17 - hep17) + tw0cm * (hm17 - hem17)) + dt*sourcep17;
	hn2[gpu_fieldn_index(x, y, z, 17)] = hnt17 - (tw0cnp * (hnp17 - hnep17) + tw0cnm * (hnm17 - hnem17)) + dt*sourcen17;
	...
	h2[gpu_fieldn_index(x, y, z, 18)] = ht18 - (tw0cp * (hp18 - hep18) + tw0cm * (hm18 - hem18)) + dt*sourcep18;
	hn2[gpu_fieldn_index(x, y, z, 18)] = hnt18 - (tw0cnp * (hnp18 - hnep18) + tw0cnm * (hnm18 - hnem18)) + dt*sourcen18;
	...
	h2[gpu_fieldn_index(x, y, z, 19)] = ht19 - (tw0cp * (hp19 - hep19) + tw0cm * (hm19 - hem19)) + dt*sourcep19;
	hn2[gpu_fieldn_index(x, y, z, 19)] = hnt19 - (tw0cnp * (hnp19 - hnep19) + tw0cnm * (hnm19 - hnem19)) + dt*sourcen19;
	...
	h2[gpu_fieldn_index(x, y, z, 20)] = ht20 - (tw0cp * (hp20 - hep20) + tw0cm * (hm20 - hem20)) + dt*sourcep20;
	hn2[gpu_fieldn_index(x, y, z, 20)] = hnt20 - (tw0cnp * (hnp20 - hnep20) + tw0cnm * (hnm20 - hnem20)) + dt*sourcen20;
	...
	h2[gpu_fieldn_index(x, y, z, 21)] = ht21 - (tw0cp * (hp21 - hep21) + tw0cm * (hm21 - hem21)) + dt*sourcep21;
	hn2[gpu_fieldn_index(x, y, z, 21)] = hnt21 - (tw0cnp * (hnp21 - hnep21) + tw0cnm * (hnm21 - hnem21)) + dt*sourcen21;
	...
	h2[gpu_fieldn_index(x, y, z, 22)] = ht22 - (tw0cp * (hp22 - hep22) + tw0cm * (hm22 - hem22)) + dt*sourcep22;
	hn2[gpu_fieldn_index(x, y, z, 22)] = hnt22 - (tw0cnp * (hnp22 - hnep22) + tw0cnm * (hnm22 - hnem22)) + dt*sourcen22;
	...
	h2[gpu_fieldn_index(x, y, z, 23)] = ht23 - (tw0cp * (hp23 - hep23) + tw0cm * (hm23 - hem23)) + dt*sourcep23;
	hn2[gpu_fieldn_index(x, y, z, 23)] = hnt23 - (tw0cnp * (hnp23 - hnep23) + tw0cnm * (hnm23 - hnem23)) + dt*sourcen23;
	...
	h2[gpu_fieldn_index(x, y, z, 24)] = ht24 - (tw0cp * (hp24 - hep24) + tw0cm * (hm24 - hem24)) + dt*sourcep24;
	hn2[gpu_fieldn_index(x, y, z, 24)] = hnt24 - (tw0cnp * (hnp24 - hnep24) + tw0cnm * (hnm24 - hnem24)) + dt*sourcen24;
	...
	h2[gpu_fieldn_index(x, y, z, 25)] = ht25 - (tw0cp * (hp25 - hep25) + tw0cm * (hm25 - hem25)) + dt*sourcep25;
	hn2[gpu_fieldn_index(x, y, z, 25)] = hnt25 - (tw0cnp * (hnp25 - hnep25) + tw0cnm * (hnm25 - hnem25)) + dt*sourcen25;
	...
	h2[gpu_fieldn_index(x, y, z, 26)] = ht26 - (tw0cp * (hp26 - hep26) + tw0cm * (hm26 - hem26)) + dt*sourcep26;
	hn2[gpu_fieldn_index(x, y, z, 26)] = hnt26 - (tw0cnp * (hnp26 - hnep26) + tw0cnm * (hnm26 - hnem26)) + dt*sourcen26;
	...
	double *temp0, double *temp1, double *temp2, double *f0bc)
{
	...
	if ((z == 0 || z == NZ - 1) && (x>=cathodeL && x<=cathodeR)) {
		// Zero charge gradient of positive charge at cathode
		h0[gpu_field0_index(x, y, z)] = h0[gpu_field0_index(x, y, neighbor)];
		h2[gpu_fieldn_index(x, y, z, 1)] = h2[gpu_fieldn_index(x, y, neighbor, 1)];
		h2[gpu_fieldn_index(x, y, z, 2)] = h2[gpu_fieldn_index(x, y, neighbor, 2)];
		h2[gpu_fieldn_index(x, y, z, 3)] = h2[gpu_fieldn_index(x, y, neighbor, 3)];
		h2[gpu_fieldn_index(x, y, z, 4)] = h2[gpu_fieldn_index(x, y, neighbor, 4)];
		h2[gpu_fieldn_index(x, y, z, 5)] = h2[gpu_fieldn_index(x, y, neighbor, 5)];
		h2[gpu_fieldn_index(x, y, z, 6)] = h2[gpu_fieldn_index(x, y, neighbor, 6)];
		h2[gpu_fieldn_index(x, y, z, 7)] = h2[gpu_fieldn_index(x, y, neighbor, 7)];
		h2[gpu_fieldn_index(x, y, z, 8)] = h2[gpu_fieldn_index(x, y, neighbor, 8)];
		h2[gpu_fieldn_index(x, y, z, 9)] = h2[gpu_fieldn_index(x, y, neighbor, 9)];
		h2[gpu_fieldn_index(x, y, z, 10)] = h2[gpu_fieldn_index(x, y, neighbor, 10)];
		h2[gpu_fieldn_index(x, y, z, 11)] = h2[gpu_fieldn_index(x, y, neighbor, 11)];
		h2[gpu_fieldn_index(x, y, z, 12)] = h2[gpu_fieldn_index(x, y, neighbor, 12)];
		h2[gpu_fieldn_index(x, y, z, 13)] = h2[gpu_fieldn_index(x, y, neighbor, 13)];
		h2[gpu_fieldn_index(x, y, z, 14)] = h2[gpu_fieldn_index(x, y, neighbor, 14)];
		h2[gpu_fieldn_index(x, y, z, 15)] = h2[gpu_fieldn_index(x, y, neighbor, 15)];
		h2[gpu_fieldn_index(x, y, z, 16)] = h2[gpu_fieldn_index(x, y, neighbor, 16)];
		h2[gpu_fieldn_index(x, y, z, 17)] = h2[gpu_fieldn_index(x, y, neighbor, 17)];
		h2[gpu_fieldn_index(x, y, z, 18)] = h2[gpu_fieldn_index(x, y, neighbor, 18)];
		h2[gpu_fieldn_index(x, y, z, 19)] = h2[gpu_fieldn_index(x, y, neighbor, 19)];
		h2[gpu_fieldn_index(x, y, z, 20)] = h2[gpu_fieldn_index(x, y, neighbor, 20)];
		h2[gpu_fieldn_index(x, y, z, 21)] = h2[gpu_fieldn_index(x, y, neighbor, 21)];
		h2[gpu_fieldn_index(x, y, z, 22)] = h2[gpu_fieldn_index(x, y, neighbor, 22)];
		h2[gpu_fieldn_index(x, y, z, 23)] = h2[gpu_fieldn_index(x, y, neighbor, 23)];
		h2[gpu_fieldn_index(x, y, z, 24)] = h2[gpu_fieldn_index(x, y, neighbor, 24)];
		h2[gpu_fieldn_index(x, y, z, 25)] = h2[gpu_fieldn_index(x, y, neighbor, 25)];
		h2[gpu_fieldn_index(x, y, z, 26)] = h2[gpu_fieldn_index(x, y, neighbor, 26)];

	}

	if ((z == 0 || z == NZ - 1) && (x >= anodeL && x <= anodeR)) {
		// Zero charge gradient of negative charge at anode
		hn0[gpu_field0_index(x, y, z)] = hn0[gpu_field0_index(x, y, neighbor)];
		hn2[gpu_fieldn_index(x, y, z, 1)] = hn2[gpu_fieldn_index(x, y, neighbor, 1)];
		hn2[gpu_fieldn_index(x, y, z, 2)] = hn2[gpu_fieldn_index(x, y, neighbor, 2)];
		hn2[gpu_fieldn_index(x, y, z, 3)] = hn2[gpu_fieldn_index(x, y, neighbor, 3)];
		hn2[gpu_fieldn_index(x, y, z, 4)] = hn2[gpu_fieldn_index(x, y, neighbor, 4)];
		hn2[gpu_fieldn_index(x, y, z, 5)] = hn2[gpu_fieldn_index(x, y, neighbor, 5)];
		hn2[gpu_fieldn_index(x, y, z, 6)] = hn2[gpu_fieldn_index(x, y, neighbor, 6)];
		hn2[gpu_fieldn_index(x, y, z, 7)] = hn2[gpu_fieldn_index(x, y, neighbor, 7)];
		hn2[gpu_fieldn_index(x, y, z, 8)] = hn2[gpu_fieldn_index(x, y, neighbor, 8)];
		hn2[gpu_fieldn_index(x, y, z, 9)] = hn2[gpu_fieldn_index(x, y, neighbor, 9)];
		hn2[gpu_fieldn_index(x, y, z, 10)] = hn2[gpu_fieldn_index(x, y, neighbor, 10)];
		hn2[gpu_fieldn_index(x, y, z, 11)] = hn2[gpu_fieldn_index(x, y, neighbor, 11)];
		hn2[gpu_fieldn_index(x, y, z, 12)] = hn2[gpu_fieldn_index(x, y, neighbor, 12)];
		hn2[gpu_fieldn_index(x, y, z, 13)] = hn2[gpu_fieldn_index(x, y, neighbor, 13)];
		hn2[gpu_fieldn_index(x, y, z, 14)] = hn2[gpu_fieldn_index(x, y, neighbor, 14)];
		hn2[gpu_fieldn_index(x, y, z, 15)] = hn2[gpu_fieldn_index(x, y, neighbor, 15)];
		hn2[gpu_fieldn_index(x, y, z, 16)] = hn2[gpu_fieldn_index(x, y, neighbor, 16)];
		hn2[gpu_fieldn_index(x, y, z, 17)] = hn2[gpu_fieldn_index(x, y, neighbor, 17)];
		hn2[gpu_fieldn_index(x, y, z, 18)] = hn2[gpu_fieldn_index(x, y, neighbor, 18)];
		hn2[gpu_fieldn_index(x, y, z, 19)] = hn2[gpu_fieldn_index(x, y, neighbor, 19)];
		hn2[gpu_fieldn_index(x, y, z, 20)] = hn2[gpu_fieldn_index(x, y, neighbor, 20)];
		hn2[gpu_fieldn_index(x, y, z, 21)] = hn2[gpu_fieldn_index(x, y, neighbor, 21)];
		hn2[gpu_fieldn_index(x, y, z, 22)] = hn2[gpu_fieldn_index(x, y, neighbor, 22)];
		hn2[gpu_fieldn_index(x, y, z, 23)] = hn2[gpu_fieldn_index(x, y, neighbor, 23)];
		hn2[gpu_fieldn_index(x, y, z, 24)] = hn2[gpu_fieldn_index(x, y, neighbor, 24)];
		hn2[gpu_fieldn_index(x, y, z, 25)] = hn2[gpu_fieldn_index(x, y, neighbor, 25)];
		hn2[gpu_fieldn_index(x, y, z, 26)] = hn2[gpu_fieldn_index(x, y, neighbor, 26)];
	}
}
...
__global__ void gpu_bc_charge(double *h0, double *h1, double *h2, double *hn0, double *hn1, double *hn2, double *temp0, double *temp1, double *temp2)
{
	...
	if ((z == 0 || z == NZ - 1) && ((x<cathodeL) || (x>cathodeR && x<anodeL) || (x>anodeR)) ) {	// boundaries at the substrates
		// positive charge
	...
	if ((z == 0 || z == NZ - 1) && (x >= cathodeL && x <= cathodeR)) {	// negative charge is zero at cathode
		double ht1 = hn2[gpu_fieldn_index(x, y, z, 1)];
		double ht2 = hn2[gpu_fieldn_index(x, y, z, 2)];
		double ht3 = hn2[gpu_fieldn_index(x, y, z, 3)];
		double ht4 = hn2[gpu_fieldn_index(x, y, z, 4)];
		double ht5 = hn2[gpu_fieldn_index(x, y, z, 5)];
		double ht6 = hn2[gpu_fieldn_index(x, y, z, 6)];
		double ht7 = hn2[gpu_fieldn_index(x, y, z, 7)];
		double ht8 = hn2[gpu_fieldn_index(x, y, z, 8)];
		double ht9 = hn2[gpu_fieldn_index(x, y, z, 9)];
		double ht10 = hn2[gpu_fieldn_index(x, y, z, 10)];
		double ht11 = hn2[gpu_fieldn_index(x, y, z, 11)];
		double ht12 = hn2[gpu_fieldn_index(x, y, z, 12)];
		double ht13 = hn2[gpu_fieldn_index(x, y, z, 13)];
		double ht14 = hn2[gpu_fieldn_index(x, y, z, 14)];
		double ht15 = hn2[gpu_fieldn_index(x, y, z, 15)];
		double ht16 = hn2[gpu_fieldn_index(x, y, z, 16)];
		double ht17 = hn2[gpu_fieldn_index(x, y, z, 17)];
		double ht18 = hn2[gpu_fieldn_index(x, y, z, 18)];
		double ht19 = hn2[gpu_fieldn_index(x, y, z, 19)];
		double ht20 = hn2[gpu_fieldn_index(x, y, z, 20)];
		double ht21 = hn2[gpu_fieldn_index(x, y, z, 21)];
		double ht22 = hn2[gpu_fieldn_index(x, y, z, 22)];
		double ht23 = hn2[gpu_fieldn_index(x, y, z, 23)];
		double ht24 = hn2[gpu_fieldn_index(x, y, z, 24)];
		double ht25 = hn2[gpu_fieldn_index(x, y, z, 25)];
		double ht26 = hn2[gpu_fieldn_index(x, y, z, 26)];
	...
	if ((z == 0 || z == NZ - 1) && (x >= anodeL && x <= anodeR)) {	// positive charge is zero at anode
		double ht1 = h2[gpu_fieldn_index(x, y, z, 1)];
		double ht2 = h2[gpu_fieldn_index(x, y, z, 2)];
		double ht3 = h2[gpu_fieldn_index(x, y, z, 3)];
		double ht4 = h2[gpu_fieldn_index(x, y, z, 4)];
...
void save_data_tecplot(FILE *fout, double time, double *rho_gpu, double *charge_gpu, double *chargen_gpu, double *phi_gpu,
	...
	for (unsigned int y = 0; y < NY; ++y) {
		for (unsigned int x = 0; x < NX; ++x) {
			...
			ux[scalar_index(x, y, 0)] = 0;// 2.0*ux[scalar_index(x, y, 1)] - ux[scalar_index(x, y, 2)];
			uy[scalar_index(x, y, 0)] = 0;//2.0*uy[scalar_index(x, y, 1)] - uy[scalar_index(x, y, 2)];
			uz[scalar_index(x, y, 0)] = 0;//2.0*uz[scalar_index(x, y, 1)] - uz[scalar_index(x, y, 2)];
			phi[scalar_index(x, y, 0)] = 2.0*phi[scalar_index(x, y, 1)] - phi[scalar_index(x, y, 2)];
			...
			ux[scalar_index(x, y, NZ - 1)] = 0;//2.0*ux[scalar_index(x, y, NZ - 2)] - ux[scalar_index(x, y, NZ - 3)];
			uy[scalar_index(x, y, NZ - 1)] = 0;//2.0*uy[scalar_index(x, y, NZ - 2)] - uy[scalar_index(x, y, NZ - 3)];
			uz[scalar_index(x, y, NZ - 1)] = 0;//2.0*uz[scalar_index(x, y, NZ - 2)] - uz[scalar_index(x, y, NZ - 3)];
			phi[scalar_index(x, y, NZ - 1)] = 2.0*phi[scalar_index(x, y, NZ - 2)] - phi[scalar_index(x, y, NZ - 3)];
			Temp[scalar_index(x, y, NZ - 1)] = 0.0;
			Temp[scalar_index(x, y, 0)] = 1.0;

			// Anode
			if (x >= anodeL && x <= anodeR) {
				phi[scalar_index(x, y, 0)] = voltage_host;
				phi[scalar_index(x, y, NZ - 1)] = voltage_host;
				charge[scalar_index(x, y, 0)] = 0;
				charge[scalar_index(x, y, NZ - 1)] = 0;
			}

			// Cathode
			if (x >= cathodeL && x <= cathodeR) {
				phi[scalar_index(x, y, 0)] = 0;
				phi[scalar_index(x, y, NZ - 1)] = 0;
				chargen[scalar_index(x, y, 0)] = 0;
				chargen[scalar_index(x, y, NZ - 1)] = 0;
			}
...
void save_data_end(FILE *fend, double time, double *rho_gpu, double *charge_gpu, double *chargen_gpu, double *phi_gpu,
	...
	for (unsigned int z = 0; z < NZ; ++z)
	{
		for (unsigned int y = 0; y < NY; ++y)
		{
			for (unsigned int x = 0; x < NX; ++x)
			{
				fprintf(fend, "%10.12f %10.12f %10.12f %10.12f %10.12f %10.12f %10.12f %10.12f %10.12f %10.12f %10.12f %10.12f\n", time,
					ux[scalar_index(x, y, z)], uy[scalar_index(x, y, z)], uz[scalar_index(x, y, z)], rho[scalar_index(x, y, z)], charge[scalar_index(x, y, z)], chargen[scalar_index(x, y, z)],
					phi[scalar_index(x, y, z)], Ex[scalar_index(x, y, z)], Ey[scalar_index(x, y, z)], Ez[scalar_index(x, y, z)], temp[scalar_index(x, y, z)]);
			}
		}
...
__host__
void write_profile(double *phi_gpu) {
	...
	//for (unsigned int y = 0; y < NY; ++y) {
	//	for (unsigned int x = 0; x < NX; ++x) {
	//		phi[scalar_index(x, y, NZ - 1)] = 2 * phi[scalar_index(x, y, NZ - 2)] - phi[scalar_index(x, y, NZ - 3)];// voltage_host;
	//		phi[scalar_index(x, y, 0)] = 2 * phi[scalar_index(x, y, 1)]- phi[scalar_index(x, y, 2)];// voltage_host;
	//	}
	//}

	FILE *f = fopen("phi_profile.dat", "wb+");

	for (unsigned int z = 0; z < NZ; ++z)
	{
		fprintf(f, "%2.14f %2.14f\n", dz_host*z, phi[scalar_index(anodeL+5,0,z)]);
	}
