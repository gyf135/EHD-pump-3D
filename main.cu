...
int main(int argc, char* argv[])
{
	...
	checkCudaErrors(cudaMalloc((void**)&phi0_gpu, mem_size_0dir));
	checkCudaErrors(cudaMalloc((void**)&phi1_gpu, mem_size_n0dir));
	checkCudaErrors(cudaMalloc((void**)&phi2_gpu, mem_size_n0dir));
    ...
    init_equilibrium(f0_gpu,f1_gpu,h0_gpu,h1_gpu, hn0_gpu, hn1_gpu, temp0_gpu, temp1_gpu, phi0_gpu, phi1_gpu, rho_gpu,charge_gpu, chargen_gpu,
		ux_gpu,uy_gpu,uz_gpu,Ex_gpu,Ey_gpu,Ez_gpu,T_gpu, phi_gpu);

	// Write a profile
	write_profile(phi_gpu);


	// initialize the electric potential (phi)
	//for (int it = 0; it < N_in_phi; it++) {
	//	LBM_Poisson(phi0_gpu, phi1_gpu, phi2_gpu, charge_gpu, chargen_gpu, phi_gpu, t);
	//}
	...
	for (int i = 0; i < NSTEPS; i++) {
		...
		if (i%NSAVE == 1) {
			save_data_tecplot(fout, t, rho_gpu, charge_gpu, chargen_gpu, phi_gpu, ux_gpu, uy_gpu, uz_gpu, Ex_gpu, Ey_gpu, Ez_gpu, T_gpu, 0);
